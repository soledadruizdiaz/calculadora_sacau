...
async function cargarCarreras() {
  try {
    ...
    parsearCatalogo(filas).forEach(p => registrarPar(p.facultad,p.carrera,false));
    $("catalog-status").textContent = `${catalogo.length} carrera(s) disponibles en el catálogo.`;
  } catch (err) { $("catalog-status").textContent = `${err.message} Subí el CSV incluido en el ZIP junto con los demás archivos.`; }
  try {
    const guardadas = JSON.parse(localStorage.getItem("sacau_catalogo_personal") || "[]");
    if (Array.isArray(guardadas)) guardadas.forEach(p => {
      if (typeof p?.facultad === "string" && typeof p?.carrera === "string") {
        registrarPar(p.facultad, p.carrera, false);
        agregados.push(p);
      }
    });
...
function guardarCatalogo() {
  try {
    localStorage.setItem("sacau_catalogo_personal", JSON.stringify(agregados));
    return true;
  } catch { return false; }
}
function registrarPar(facultad, carrera, guardar = true) {
  ...
  if (guardar && !agregados.some(p => normalizar(p.facultad) === normalizar(f) && normalizar(p.carrera) === normalizar(carrera)))
    agregados.push({facultad:f,carrera});
  if (guardar) guardarCatalogo();
}
function mostrarCarreras() {
  ...
  select.add(new Option("Agregar otra carrera…", "__other__"));
  select.value = [...select.options].some(o => o.value === previo) ? previo : "";
  $("new-career-wrap").hidden = select.value !== "__other__";
}
function guardarCarrera() {
  ...
  const facultad = $("faculty-select").value;
  if (!facultad || facultad === "__other__") {$("career-status").textContent = "Primero seleccioná una facultad."; return;}
  registrarPar(facultad, nombre);
  mostrarCarreras();
  ...
  if (!guardarCatalogo()) $("career-status").textContent = "Carrera agregada para esta sesión. El navegador no permitió guardarla para la próxima visita.";
  limpiar();
}
function guardarFacultad() {
  ...
  $("new-faculty").value = "";
  mostrarCarreras();
  $("career-status").textContent = "Facultad agregada. Agregá su carrera o cargá un catálogo.";
  limpiar();
}
...
$("faculty-select").addEventListener("change", () => {$("new-faculty-wrap").hidden = $("faculty-select").value !== "__other__"; mostrarCarreras(); $("career-status").textContent = ""; limpiar();});
