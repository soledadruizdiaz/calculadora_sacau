...
La modalidad (Presencial o A Distancia) y el artículo (42 o 43) se seleccionan en la página. Los combos de Facultad y Carrera muestran **exclusivamente** los valores del CSV del repositorio; para agregar una opción, editar y confirmar ese archivo en GitHub.
...
El Excel descargado incluye **Datos de la carrera**, **Créditos SACAU** y **Resumen por nivel**, con totales de HIP, HTA, Total y CRE. En pantalla y en PDF aparecen las mismas columnas de totales; el PDF incorpora el logo institucional.
...
2. Subir los seis archivos descomprimidos (`index.html`, `styles.css`, `app-sacau-v8.js`, `UA_logo_sinfondo.png`, `catalogo_facultades_carreras.csv`, `README.md`) a la **raíz** del repositorio, mediante **Add file → Upload files → Commit changes**. No subir el ZIP como único archivo. Si ya publicaste una versión anterior, reemplazá `index.html` y `styles.css`, agregá `app-sacau-v8.js` y asegurate de que el logo y el CSV estén en la raíz. El JavaScript anterior puede eliminarse.
